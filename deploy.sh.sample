...
while true; do
  case $STEP in
    ...
    5)
      ...
      (
        echo "5"; echo "# Adicionando arquivos..."
        git add . 2>"$ERR_FILE" || exit 1

        echo "15"; echo "# Criando commit em develop..."
        git commit -m "$COMMIT_MSG" 2>"$ERR_FILE" || exit 1

        echo "30"; echo "# Enviando develop para o repositório..."
        git push origin develop 2>"$ERR_FILE" || exit 1

        echo "45"; echo "# Trocando para a branch master..."
        git checkout master 2>"$ERR_FILE" || exit 1

        echo "60"; echo "# Fazendo merge de develop em master..."
        if ! git merge develop --no-edit 2>"$ERR_FILE"; then
          echo "# Conflito no merge! Abortando e voltando para develop..." >> "$ERR_FILE"
          git merge --abort 2>>"$ERR_FILE"
          git checkout develop 2>>"$ERR_FILE"
          exit 1
        fi

        echo "75"; echo "# Criando tag $NEW_TAG em master..."
        git tag -a "$NEW_TAG" -m "$NEW_TAG" 2>"$ERR_FILE" || exit 1

        echo "90"; echo "# Enviando master e tag para o repositório..."
        git push origin master --tags 2>"$ERR_FILE" || exit 1

        echo "95"; echo "# Voltando para a branch develop..."
        git checkout develop 2>"$ERR_FILE" || exit 1
      ...
      if [ "${PIPESTATUS[0]}" -ne 0 ]; then
        ...
        rm -f "$ERR_FILE"
        if echo "$ERR_MSG" | grep -q "Conflito no merge"; then
          whiptail --title "Conflito no merge" \
            --msgbox "O merge de develop em master gerou conflito.\n\nO merge foi abortado e a branch develop foi restaurada automaticamente — resolva os conflitos manualmente em develop e tente novamente.\n\n$ERR_MSG" \
            18 70
        else
